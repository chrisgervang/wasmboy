...
class WasmBoyLibService {
  ...
  play() {
    const playTask = async () => {
      ...
      if (this.options.sendDataToFirebase) {
        this.options.sendDataToFirebase([{ address: 0xd123, value: 0x05 }]);
      }
    };
  ...
  simulateDataCollection() {
    if (this.options.sendDataToFirebase) {
      this.options.sendDataToFirebase([{ address: 456, value: 7 }]);
    }
  }
