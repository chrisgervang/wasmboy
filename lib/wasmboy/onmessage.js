...
export function libWorkerOnMessage(event) {
  ...
  switch (eventData.message.type) {
    case WORKER_MESSAGE_TYPE.ON_MEMORY_WRITE: {
      if (eventData.message.address >= 0xdcb4 && eventData.message.address <= 0xdcb8) {
        this.options.sendDataToFirebase(eventData.message);
      }
    }
