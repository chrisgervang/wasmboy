    ...
    switch (eventData.message.type) {
      ...
      case WORKER_MESSAGE_TYPE.INSTANTIATE_WASM: {
        const instantiateTask = async () => {
          const onMemoryWrite = (gameboyOffset, value) => {
            postMessage(
              getSmartWorkerMessage({
                type: WORKER_MESSAGE_TYPE.ON_MEMORY_WRITE,
                response: { gameboyOffset, value }
              })
            );
          };
          ...
          // Only One response will be used on build time
          // Using Babel plugin to filter imports
          response = await getWasmBoyWasmCore(isInBrowser, onMemoryWrite);
